...
hdlis = set([ 'АННОТАЦИЯ', 'ВВЕДЕНИЕ', 'ЗАКЛЮЧЕНИЕ', 'ВЫВОДЫ', 'СПИСОК-ИСПОЛЬЗОВАННЫХ-ИСТОЧНИКОВ' ])
headers_used = set([])
...
while i < len(blocks):
    ...
    if btype == 'Header' and order == 1:
        ...
        if not unnumbered:
            ...
            if (refname.strip()).upper() == 'ВЫВОДЫ' or (refname.strip()).upper() == 'ЗАКЛЮЧЕНИЕ':
                headers_used.add('ВЫВОДЫ')
                headers_used.add('ЗАКЛЮЧЕНИЕ')
            else:
                headers_used.add((refname.strip()).upper())
...
not_used = hdlis - headers_used
if 'ВЫВОДЫ' in not_used and 'ЗАКЛЮЧЕНИЕ' in not_used:
    eprint(E+'отсутствуют разделы ВЫВОДЫ либо ЗАКЛЮЧЕНИЕ.')
    not_used.remove('ВЫВОДЫ')
    not_used.remove('ЗАКЛЮЧЕНИЕ')

for h in not_used:
    if h == 'АННОТАЦИЯ':
        eprint(W+"отсутствует раздел АННОТАЦИЯ (необязателен для всех работ, кроме дипломных)")
    else:
        eprint(E+'отсутствует раздел ' + h)
